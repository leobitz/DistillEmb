...
class WordsTagger:
    ...
    def __call__(self, sentences, model,  begin_tags="BS", device='cuda'):
        """predict texts
        ...
        try:
            if type(sentences) == type([]):
                sent_tensor = np.array([self.preprocessor.sent_to_vector(s) for s in sentences])
